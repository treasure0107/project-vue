...
function showDate() {
	...
	document.getElementById('dateOuter').style.left = "50%";
	document.getElementById('dateOuter').style.top = "50%";
 ...
 * */
//日历最外层div样式
var outerStyle = "display: none;position: absolute;width: 314px;margin-left:-53px;margin-top:-134px;border: 1px solid #F7F7F7;border-radius: 5px;background-color: #F7F7F7;color: #666666;padding-bottom: 20px;";
//日历控件最外层div
document.getElementById("appointment_window").innerHTML+='<div style="' + outerStyle + '" id="dateOuter"></div>';  
// document.getElementById('appointment_window').write('<div style="' + outerStyle + '" id="dateOuter"></div>')
	//创建日历样式 
function createDate(thisYear, thisMonth) {
	...
	//关闭日历显示
	createDoc += '<span id="closeDate" onClick="hidDate()" style="float: right;font-size: 25px;margin: -8px 3px 0 0;cursor: pointer;color:#666666">×</span></p></div>';;
	createDoc += '<div style="margin-bottom: 8px;color:#ff5958">';
	//上一月
	createDoc += '<span id="lastMonth" onclick="lastMonthClick()" style="margin: 0 20px 0 25px;cursor:pointer;color:#666666"><</span>';
	//创建年份下拉框[1900-2099]年
	createDoc += '<select id ="selectYear" class="selectStyle" onchange="changeYearAndMonth()" style="color:#666666">';
	for(var i = 1900; i <= 2099; i++) {
		createDoc += "<option value=" + i + ">" + i + "</option>";
	}
	createDoc += "</select>年";
	//创建月份下拉框
	createDoc += '<select id ="selectMonth" class="selectStyle" onchange="changeYearAndMonth()" style="color:#666666">';
	for(var i = 1; i <= 12; i++) {
		createDoc += "<option value=" + i + ">" + i + "</option>";
	}
	createDoc += "</select>月";
	//下一月
	createDoc += '<span id="nextMonth" onClick="nextMonthClick()" style="float: right;margin-right: 25px;cursor:pointer;color:#666666">></span></div>';
	//创建星期
	createDoc += '<div class="everyWeekDay">';
	for(var i = 0; i < weekDays.length; i++) {
		if(weekDays[i] == "日" || weekDays[i] == "六") {
			createDoc += '<span class="weekday" style="color:#666666;">' + weekDays[i] + '</span>'
		} else {
	...
	for(var i = 1; i < getThisMonthDay(thisYear, thisMonth) + 1; i++) {
		if(thisYear==nowYear && thisMonth == nowMonth && i== nowDay){
			...
			if(getThisWeekDay(thisYear,thisMonth,i) == 6 || getThisWeekDay(thisYear,thisMonth,i) == 0){
				//今天是周末
				createDoc+='<span onclick="setInput('+i+')" class="days" style="background-color:#ff5958;color:#FFFFFF;cursor:pointer;">'+i+'</span>';  
			}else{  
		      	createDoc+='<span onClick="setInput('+i+')" class="days" style="background-color:#ff5958;color:#FFFFFF;cursor:pointer;">'+i+'</span>';  
		    }  
		}else{  
		     //周末变为红色
		    if(getThisWeekDay(thisYear,thisMonth,i)==6 || getThisWeekDay(thisYear,thisMonth,i)==0){  
	      		createDoc+='<span id="weekends" onClick="setInput('+i+')" class="days" onmouseover="mouseOver(this);" onmouseout="mouseOut(this)" style="color:#666666;cursor:pointer;">'+i+'</span>';
		    }else{
...
function mouseOver(obj){
	if(obj.id == "weekends"){
		//若为周末，边框样式为红色
		obj.style.border="1px solid #ff5958"; 
	}else{
		obj.style.border="1px solid #ff5958";
	}
}
